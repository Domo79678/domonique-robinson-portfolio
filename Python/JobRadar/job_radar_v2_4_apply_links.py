...
def clean_url(url):
    """Clean and normalize URLs found inside job alert emails."""
    if not url:
        return ""
...
def get_link_domain(url):
    """Return a readable domain for a link."""
    try:
...
def extract_email_links(raw_body):
    """Extract usable links from the original HTML email body."""
    soup = BeautifulSoup(raw_body, "html.parser")
    links = []
    seen = set()

    blocked_terms = [
        "unsubscribe", "privacy", "preferences", "settings", "manage alerts",
        "email", "help", "support", "view in browser", "view as web page"
    ]

    for anchor in soup.find_all("a"):
        href = clean_url(anchor.get("href", ""))
        text = clean_export_text(anchor.get_text(" "))
...
def choose_best_apply_link(email, job_title="", company="", source=""):
    """Choose the best available apply link for a job from the email links.

    Many alerts use redirect links back to LinkedIn, Google, Indeed, or CareerBuilder.
    That is okay for V2.4 because the goal is to get back to the original posting quickly.
    """
    links = email.get("links", []) or []
    if not links:
        return "", "No link found", 0

    title_words = [w for w in re.findall(r"[a-z0-9]+", job_title.lower()) if len(w) >= 4]
    company_words = [w for w in re.findall(r"[a-z0-9]+", company.lower()) if len(w) >= 4]
    ...
    for link in links:
        ...
        # Prefer job-board links and career-site redirects.
        if any(term in domain for term in ["linkedin", "google", "indeed", "careerbuilder", "ziprecruiter"]):
            score += 20

        if any(term in url_lower for term in ["/jobs/", "job", "career", "apply", "position", "opening"]):
            score += 25

        if source_lower and source_lower in domain:
            score += 10

        # Match link text against the job title/company when possible.
        score += sum(4 for word in title_words if word in link_text)
        score += sum(3 for word in company_words if word in link_text)

        if any(phrase in link_text for phrase in ["apply", "view job", "see job", "view role", "learn more"]):
            score += 15
    ...
    if not best_link:
        return "", "No link found", len(links)

    return best_link.get("url", ""), best_link.get("domain", "Unknown"), len(links)
...
def read_full_email(service, message_id):
    ...
    links = extract_email_links(raw_body)
...
def build_job(job_title, company, location, source, email):
    apply_link, apply_link_source, link_count = choose_best_apply_link(
        email, job_title=job_title, company=company, source=source
    )
...
def score_job(job):
    ...
    job["interview_date"] = ""
    job["follow_up_date"] = ""
    job["job_link"] = job.get("job_link", "")
    job["apply_link_source"] = job.get("apply_link_source", "")
    job["email_link_count"] = job.get("email_link_count", 0)
    job["notes"] = ""
...
def apply_application_tracker(scored_jobs):
    ...
    for job in scored_jobs:
        ...
        if saved:
            ...
            job_link = saved.get("job_link", "") or job.get("job_link", "")
...
def export_reports(scored_jobs, recruiter_leads):
    ...
    preferred_columns = [
        ...
        "salary", "follow_up_date", "last_updated", "job_link", "apply_link_source", "email_link_count", "notes",
