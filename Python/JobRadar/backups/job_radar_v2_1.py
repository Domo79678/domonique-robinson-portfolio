...
def get_location_status(job_text):
    ...
    if any(country in text for country in INTERNATIONAL_LOCATIONS):
...
def score_job(job):
    ...
    company_bonus, company_match = get_company_bonus(job.get("company", ""))
    if company_bonus > 0:
        score += company_bonus
        reasons.append(f"Target company: {company_match}")
...
def get_stage_from_status(status):
    """Convert a detailed application status into a broader pipeline stage."""
    if not status:
        return "Prospect"
    return STATUS_TO_STAGE.get(status, "Prospect")
...
def apply_application_tracker(scored_jobs):
    ...
    for job in scored_jobs:
        ...
        if saved:
            ...
            stage = saved.get("stage", "") or get_stage_from_status(status)
            date_applied = saved.get("date_applied", "")
            follow_up = saved.get("follow_up", "")
            last_updated = saved.get("last_updated", "")
            resume_used = saved.get("resume_used", "") or job.get("recommended_resume", "")
            ...
            status = "Not Applied"
            stage = get_stage_from_status(status)
            date_applied = ""
            follow_up = ""
            last_updated = today
...
def build_dashboard_df(df, recruiter_df):
    metrics = [
        ...
        ("Applications Submitted", len(df[df["application_status"] == "Applied"]) if "application_status" in df.columns else 0),
        ("Recruiter Contacted", len(df[df["application_status"] == "Recruiter Contacted"]) if "application_status" in df.columns else 0),
        ("Phone Screens", len(df[df["application_status"] == "Phone Screen"]) if "application_status" in df.columns else 0),
        ("Interviews", len(df[df["application_status"].isin(["Interview Scheduled", "Interview Complete", "Final Interview"])]) if "application_status" in df.columns else 0),
        ("Offers", len(df[df["application_status"] == "Offer"]) if "application_status" in df.columns else 0),
        ("Accepted", len(df[df["application_status"] == "Accepted"]) if "application_status" in df.columns else 0),
        ("Rejected", len(df[df["application_status"] == "Rejected"]) if "application_status" in df.columns else 0),
    ]
...
def export_reports(scored_jobs, recruiter_leads):
    ...
    preferred_columns = [
        ...
        "recommended_resume", "location_status", "company_bonus",
        "target_strength", "recommendation", "apply_priority",
        "application_status", "application_stage", "application_date",
        "follow_up_date", "last_updated", "resume_used", "cover_letter_used",
        "recruiter_name", "recruiter_email", "interview_date", "salary",
...
def main():
    ...
    for job in scored_jobs[:15]:
        ...
        print(f"Application Status: {job.get('application_status', '')}")
        print(f"Application Stage: {job.get('application_stage', '')}")
        print(f"Resume Used: {job.get('resume_used', '')}")
