...
def build_daily_action_center_df(df, recruiter_df, recruiter_crm_df, skill_gap_df):
    """Build a simple daily action center so JobRadar ends with decisions, not just reports."""
    rows = []

    def add(section, item, value, action=""):
        rows.append({
            "section": section,
            "item": item,
            "value": value,
    ...
    total_jobs = len(df)
    apply_now_count = len(df[df["recommendation"] == "Apply Now"]) if "recommendation" in df.columns else 0
    high_priority_count = len(df[df["recommendation"] == "High Priority"]) if "recommendation" in df.columns else 0
    weekend_count = len(df[df["recommendation"].isin(["Apply Now", "High Priority"])]) if "recommendation" in df.columns else 0
    recruiter_count = len(recruiter_df)

    add("Summary", "Jobs Found", total_jobs, "Use Apply Now and High Priority first")
    add("Summary", "Apply Today", apply_now_count, "Start with the highest score jobs")
    add("Summary", "High Priority", high_priority_count, "Review after Apply Now list")
    add("Summary", "Weekend Apply List", weekend_count, "Use this list for focused application blocks")
    add("Summary", "Recruiter Leads", recruiter_count, "Reply to high priority recruiter messages first")

    if "application_stage" in df.columns:
        applied_count = len(df[df["application_stage"].isin(["Applied", "Recruiter Contacted", "Phone Screen", "Interview Scheduled", "Interview Complete", "Final Interview", "Offer", "Accepted"])] )
        interview_count = len(df[df["application_stage"].isin(["Phone Screen", "Interview Scheduled", "Interview Complete", "Final Interview"])] )
        add("Pipeline", "Applications In Progress", applied_count, "Keep tracker updated after each application")
        add("Pipeline", "Interviews / Screens", interview_count, "Prepare STAR stories for these roles")

    # Top jobs to apply to today
    top_jobs = df[df["recommendation"].isin(["Apply Now", "High Priority"])] if "recommendation" in df.columns else df
    if not top_jobs.empty:
        top_jobs = top_jobs.sort_values(by="score", ascending=False).head(5)
        for _, job in top_jobs.iterrows():
            title = job.get("job_title", "")
            company = job.get("company", "")
            score = job.get("score", "")
            resume = job.get("recommended_resume", "")
            add("Apply Today", f"{title} at {company}", f"Score {score}", f"Use {resume} and apply today")

    # Recruiter follow-up actions
    if recruiter_crm_df is not None and not recruiter_crm_df.empty:
        crm = recruiter_crm_df.copy()
        if "relationship_score" in crm.columns:
            crm["relationship_score_num"] = pd.to_numeric(crm["relationship_score"], errors="coerce").fillna(0)
            crm = crm.sort_values(by="relationship_score_num", ascending=False)
        for _, lead in crm.head(5).iterrows():
            recruiter = lead.get("recruiter_name", lead.get("sender", "Recruiter"))
            priority = lead.get("priority", "")
            action = lead.get("suggested_action", "Review and respond if relevant")
            add("Recruiter Follow Up", recruiter, priority, action)

    # Top skill gaps
    if skill_gap_df is not None and not skill_gap_df.empty:
        for _, gap in skill_gap_df.head(5).iterrows():
            skill = gap.get("skill", "")
            count = gap.get("job_count", "")
            priority = gap.get("priority", "")
            action = gap.get("suggested_action", "Review this skill")
            add("Skill Gaps", skill, f"{count} jobs / {priority}", action)

    return pd.DataFrame(rows)


def build_daily_brief_text(export_info, daily_action_df):
    """Create a terminal-friendly daily brief."""
    def metric(name, default=0):
        return export_info.get(name, default)

    lines = []
    lines.append("=" * 55)
    lines.append("                 JOBRADAR DAILY BRIEF")
    lines.append("=" * 55)
    lines.append(f"Jobs Found:          {metric('total')}")
    lines.append(f"Apply Now:           {metric('apply_count')}")
    lines.append(f"High Priority:       {metric('high_count')}")
    lines.append(f"Weekend Apply List:  {metric('weekend_count')}")
    lines.append(f"Recruiter Leads:     {metric('recruiter_count')}")
    lines.append("")

    if daily_action_df is not None and not daily_action_df.empty:
        apply_rows = daily_action_df[daily_action_df["section"] == "Apply Today"].head(3)
        recruiter_rows = daily_action_df[daily_action_df["section"] == "Recruiter Follow Up"].head(3)
        skill_rows = daily_action_df[daily_action_df["section"] == "Skill Gaps"].head(3)

        if not apply_rows.empty:
            lines.append("Top Apply Actions:")
            for _, row in apply_rows.iterrows():
                lines.append(f"- {row['item']} | {row['value']}")
            lines.append("")

        if not recruiter_rows.empty:
            lines.append("Recruiter Follow Ups:")
            for _, row in recruiter_rows.iterrows():
                lines.append(f"- {row['item']} | {row['suggested_action']}")
            lines.append("")
    ...
    lines.append("Next Best Move: Open reports/job_radar.xlsx and start with the Weekend Apply List.")
    lines.append("=" * 55)
    return "\n".join(lines)
...
def export_reports(scored_jobs, recruiter_leads):
    ...
    daily_action_path = "reports/daily_action_center.csv"
    daily_action_excel_path = "reports/daily_action_center.xlsx"
    daily_action_text_path = "reports/daily_brief.txt"
    ...
    daily_action_df = build_daily_action_center_df(df, recruiter_df, recruiter_crm_df, skill_gap_df)
    ...
    daily_action_df.to_csv(daily_action_path, index=False)
    ...
    daily_brief_text = build_daily_brief_text(brief_preview_info, daily_action_df)
    with open(daily_action_text_path, "w", encoding="utf-8") as brief_file:
        brief_file.write(daily_brief_text)
    ...
    with pd.ExcelWriter(daily_action_excel_path, engine="openpyxl") as writer:
        daily_action_df.to_excel(writer, sheet_name="Daily Action Center", index=False)
        workbook = writer.book
        for sheet_name in workbook.sheetnames:
            style_worksheet(workbook[sheet_name])
    ...
    return {
        ...
        "daily_action": daily_action_path,
        "daily_action_excel": daily_action_excel_path,
        "daily_brief": daily_action_text_path,
        "daily_brief_text": daily_brief_text,
        "excel": excel_path,
        "total": len(df),
        "apply_count": len(apply_df),
        "high_count": len(high_df),
...
def main():
    ...
    print(f"- {export_info['daily_action']}")
    print(f"- {export_info['daily_action_excel']}")
    print(f"- {export_info['daily_brief']}")
    print(f"- {export_info['excel']}\n")

    print(export_info.get("daily_brief_text", ""))
    print()
