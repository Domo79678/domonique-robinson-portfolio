...
def open_path(path):
    if not os.path.exists(path):
        messagebox.showwarning("File not found", f"I could not find:\n\n{path}\n\nRun JobRadar first to generate this file.")
        return
...
def run_jobradar():
    ...
    result = subprocess.run(
        ...
        if result.returncode == 0:
            ...
            messagebox.showinfo("JobRadar Complete", "JobRadar finished running. Your reports were updated.")
        else:
            status_label.config(text="Scan failed. Check error message.")
            messagebox.showerror("JobRadar Error", result.stderr[-3000:] or result.stdout[-3000:])

    except subprocess.TimeoutExpired:
        status_label.config(text="Scan timed out.")
        messagebox.showerror("Timeout", "JobRadar took longer than 5 minutes. Try running it from VS Code to see details.")
    except Exception as exc:
...
run_button = tk.Button(root, text="Run JobRadar Scan", font=("Segoe UI", 12, "bold"), height=2, width=28, command=run_jobradar)
run_button.pack(pady=8)

for label, path in REPORT_PATHS.items():
    button = tk.Button(root, text=f"Open {label}", font=("Segoe UI", 10), width=28, command=lambda p=path: open_path(p))
    button.pack(pady=5)

folder_button = tk.Button(root, text="Open Reports Folder", font=("Segoe UI", 10), width=28, command=open_reports_folder)
